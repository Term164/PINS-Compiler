...
public class SynAn implements AutoCloseable {
	...
	private int cache = 0; // Expressions need to evaluate at the end. So they consume one more token
	...
	private ASTs<AstDecl> parsePRG_(ArrayList<AstDecl> declarations){
		...
		switch (peek()) {
			...
			case EOF -> {return new ASTs<AstDecl>(currentSymbol.location, declarations);}
			default -> error(currentSymbol, "outside scope");
		}
		throw new Report.Error("Internal error when constructing the abstract syntax tree at recursive prg");
	}

	// Grammar update (Where must not call prg_)

	private ASTs<AstDecl> parseWhere(ArrayList<AstDecl> declarations){
		read();
		switch (peek()) {
			case TYP, VAR, FUN -> {
				declarations.add(parseDECL());
				return parseWhere(declarations);
			}
			case RB -> {return new ASTs<AstDecl>(currentSymbol.location, declarations);}
			default -> error(currentSymbol, "outside scope");
		}
		throw new Report.Error("Internal error when constructing the abstract syntax tree at recursive where");
	}
	...
	private AstExpr parseTypeCast(AstExpr left){
		switch (peek()){
			...
			case WHERE -> {
				...
				read();
				declarations.add(parseDECL());
				ASTs<AstDecl> decls = parseWhere(declarations);
		...
		if (expectedErrorMode){
			...
			if (symbol.lexeme.length() == 1){
				...
				if (symbol.location == null){
					throw new Report.Error("Missing closing statement at the end of file.");
				}
