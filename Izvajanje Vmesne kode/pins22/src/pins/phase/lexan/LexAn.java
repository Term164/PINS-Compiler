...
public class LexAn implements AutoCloseable {
	...
	private int previouslyReadComment = 0;
	...
	private int readNext(boolean not){
		if (not){
			falseRead = false;
			return previouslyReadComment;
		}
		else {
			try {
				int current = srcFile.read();
